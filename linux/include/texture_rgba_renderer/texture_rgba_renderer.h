...
struct TextureRgbaPrivate
{
  ...
  gsize current_reading_index = 0;
  gboolean buffer_ready = false;
  gint32 video_width[2] = {0, 0};
  gint32 video_height[2] = {0, 0};
  GMutex mutex;
};
...
static void texture_rgba_class_dispose(GObject* self) {
  printf("texture_rgba_class_dispose\n");
  fflush(stdout);
  g_free(self);
}
...
static void texture_rgba_class_init(TextureRgbaClass *klass)
{
  ...
  G_OBJECT_CLASS(klass)->dispose = texture_rgba_class_dispose;
}
